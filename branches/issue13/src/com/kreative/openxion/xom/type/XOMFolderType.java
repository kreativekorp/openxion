...
public class XOMFolderType extends XOMSimpleDataType<XOMFile> {
	...
	public boolean canGetInstanceByIndex(XNContext ctx, int index) {
		...
		List<? extends XOMVariant> theXFiles = ((XOMList)getMassInstance(ctx)).toList();
	...
	public XOMVariant getChildMassVariant(XNContext ctx, XOMVariant parent) {
		if (!ctx.allow(XNSecurityKey.FILE_SYSTEM_READ, "Operation", "GetFolders", "Parent", parent.toTextString(ctx)))
			throw new XNScriptError("Security settings do not allow file system access");
	...
	public boolean canGetChildVariantByIndex(XNContext ctx, XOMVariant parent, int startIndex, int endIndex) {
		return ctx.allow(XNSecurityKey.FILE_SYSTEM_READ, "Operation", "GetFolder", "Parent", parent.toTextString(ctx), "StartIndex", Integer.toString(startIndex), "EndIndex", Integer.toString(endIndex));
	}
	public boolean canGetChildVariantByName(XNContext ctx, XOMVariant parent, String name) {
		if (ctx.allow(XNSecurityKey.FILE_SYSTEM_READ, "Operation", "GetFolder", "Parent", parent.toTextString(ctx), "Name", name) && XOMFolderType.instance.canMakeInstanceFrom(ctx, parent)) {
			File theDir = XOMFolderType.instance.makeInstanceFrom(ctx, parent).toFile();
	...
	public XOMVariant getChildVariantByName(XNContext ctx, XOMVariant parent, String name) {
		if (!ctx.allow(XNSecurityKey.FILE_SYSTEM_READ, "Operation", "GetFolder", "Parent", parent.toTextString(ctx), "Name", name))
			throw new XNScriptError("Security settings do not allow file system access");
		if (XOMFolderType.instance.canMakeInstanceFrom(ctx, parent)) {
			File theDir = XOMFolderType.instance.makeInstanceFrom(ctx, parent).toFile();
			File theFile = new File(theDir, name);
	...
	public boolean canCreateChildVariantByName(XNContext ctx, XOMVariant parent, String name) {
		return (ctx.allow(XNSecurityKey.FILE_SYSTEM_WRITE, "Operation", "MkDir", "Parent", parent.toTextString(ctx), "Name", name) && XOMFolderType.instance.canMakeInstanceFrom(ctx, parent));
	}
	public XOMVariant createChildVariantByName(XNContext ctx, XOMVariant parent, String name) {
		if (!ctx.allow(XNSecurityKey.FILE_SYSTEM_WRITE, "Operation", "MkDir", "Parent", parent.toTextString(ctx), "Name", name))
			throw new XNScriptError("Security settings do not allow file system access");
		if (XOMFolderType.instance.canMakeInstanceFrom(ctx, parent)) {
			try {
				File theDir = XOMFolderType.instance.makeInstanceFrom(ctx, parent).toFile();
