...
public abstract class XOMSimpleDataType<IT extends XOMVariant> extends XOMDataType<IT> {
	...
	public final boolean canMakeInstanceFrom(XNContext ctx, XOMVariant instance) {
		...
		if (instanceClass.isAssignableFrom(instance.getClass())) {
			...
			List<XOMVariant> l = ((XOMList)instance).toList();
			if (l.size() == 1) {
				if (instanceClass.isAssignableFrom(l.get(0).getClass())) {
					return true;
				} else {
					return canMakeInstanceFromImpl(ctx, l.get(0).asGiven());
				}
			} else {
				return canMakeInstanceFromImpl(ctx, instance);
			}
		} else if (instance instanceof XOMListChunk) {
			List<XOMVariant> l = ((XOMListChunk)instance).toList(ctx);
			if (l.size() == 1) {
				if (instanceClass.isAssignableFrom(l.get(0).getClass())) {
					return true;
				} else {
		...
		} else if (instance instanceof XOMDictionaryChunk) {
			XOMVariant h = ((XOMDictionaryChunk)instance).getContents(ctx);
			if (instanceClass.isAssignableFrom(h.getClass())) {
				return true;
			} else {
				return canMakeInstanceFromImpl(ctx, h.asGiven());
			}
		} else {
			return canMakeInstanceFromImpl(ctx, instance);
		}
		...
		if (instanceClass.isAssignableFrom(instance.getClass())) {
			...
			if (l.size() == 1) {
				if (instanceClass.isAssignableFrom(l.get(0).getClass())) {
					...
					return makeInstanceFromImpl(ctx, l.get(0).asGiven());
				}
			} else {
			...
			XOMVariant h = ((XOMDictionaryChunk)instance).getContents(ctx);
			if (instanceClass.isAssignableFrom(h.getClass())) {
				return instanceClass.cast(h);
			} else {
				return makeInstanceFromImpl(ctx, h.asGiven());
			}
		} else {
			return makeInstanceFromImpl(ctx, instance);
		}
