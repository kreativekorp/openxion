...
public class XNAudioModule extends XNModule {
	...
	private final Command c_dial = new Command() {
		public XOMVariant executeCommand(XNInterpreter interp, XNContext ctx, String commandName, List<XNExpression> parameters) {
			...
			if (parameters.size() > 1) {
				...
				String number = interp.evaluateExpression(parameters.get(0)).toTextString(ctx);
				String command;
				if (parameters.size() > 2) {
					command = interp.evaluateExpression(parameters.get(2)).toTextString(ctx);
				} else {
					command = "ATDT";
				}
	...
	private final Command c_speak = new Command() {
		public XOMVariant executeCommand(XNInterpreter interp, XNContext ctx, String commandName, List<XNExpression> parameters) {
			...
			String phrase = interp.evaluateExpression(parameters.get(0)).toTextString(ctx);
			String voice;
			if (parameters.size() == 3) {
				voice = interp.evaluateExpression(parameters.get(2)).toTextString(ctx);
	...
	private final Command c_stop = new Command() {
		public XOMVariant executeCommand(XNInterpreter interp, XNContext ctx, String commandName, List<XNExpression> parameters) {
			...
			String what = interp.evaluateExpression(parameters.get(0)).toTextString(ctx);
