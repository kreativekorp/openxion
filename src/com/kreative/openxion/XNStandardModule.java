...
public class XNStandardModule extends XNModule {
	...
	private XNStandardModule() {
		...
		functions.put("descriptor", f_descriptor);
	...
	private static final Command c_delete = new Command() {
		public XOMVariant executeCommand(XNInterpreter interp, XNContext ctx, String commandName, List<XNExpression> parameters) {
			...
			if (v.canDelete(ctx)) v.delete(ctx);
				...
				v = interp.evaluateExpressionString(v.toTextString(ctx));
				if (v.canDelete(ctx)) v.delete(ctx);
				else {
	...
	private static final Command c_put = new Command() {
		public XOMVariant executeCommand(XNInterpreter interp, XNContext ctx, String commandName, List<XNExpression> parameters) {
			...
			if (parameters.size() < 3) {
				...
				XOMVariant pval = interp.evaluateExpression(parameters.get(5)).asPrimitive(ctx);
	...
	private static final Command c_set = new Command() {
		public XOMVariant executeCommand(XNInterpreter interp, XNContext ctx, String commandName, List<XNExpression> parameters) {
			...
			XOMVariant object = interp.evaluateExpression(parameters.get(2));
			XOMVariant value = interp.evaluateExpression(parameters.get(4)).asPrimitive(ctx);
	...
	private static final Command c_close = new Command() {
		public XOMVariant executeCommand(XNInterpreter interp, XNContext ctx, String commandName, List<XNExpression> parameters) {
			...
			XOMVariant object = interp.evaluateExpression(parameters.get(0)).asPrimitive(ctx);
			XOMVariant opener = (parameters.size() > 2) ? interp.evaluateExpression(parameters.get(2)).asPrimitive(ctx) : null;
			if (opener != null) {
	...
	private static final Command c_read = new Command() {
		public XOMVariant executeCommand(XNInterpreter interp, XNContext ctx, String commandName, List<XNExpression> parameters) {
			...
			XOMVariant obj = interp.evaluateExpression(parameters.get(1)).asPrimitive(ctx);
	...
	private static final Command c_truncate = new Command() {
		public XOMVariant executeCommand(XNInterpreter interp, XNContext ctx, String commandName, List<XNExpression> parameters) {
			...
			XOMVariant obj = interp.evaluateExpression(parameters.get(0)).asPrimitive(ctx);
			XNIOManager io = ctx.getIOManager(obj);
			if (io == null) {
	...
	private static final Command c_write = new Command() {
		public XOMVariant executeCommand(XNInterpreter interp, XNContext ctx, String commandName, List<XNExpression> parameters) {
			...
			XOMVariant content = interp.evaluateExpression(parameters.get(0)).asPrimitive(ctx);
			XOMVariant obj = interp.evaluateExpression(parameters.get(2)).asPrimitive(ctx);
			XNIOManager io = ctx.getIOManager(obj);
			if (io == null) {
	...
	private static final Function f_descriptor = new Function() {
		public XOMVariant evaluateFunction(XNContext ctx, String functionName, XNModifier modifier, XOMVariant parameter) {
			if (parameter == null) return new XOMString("empty");
			else return new XOMString(parameter.asObject(ctx).toLanguageString());
		}
	};
